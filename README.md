...
### shopLinks (쿠팡 상품 링크)
`"재료이름": { "url": "쿠팡 파트너스 링크", "label": "버튼에 보일 상품명", "image": "상품 썸네일 이미지 URL (선택)" }`
재료 이름은 앱에 있는 이름과 똑같아야 해요 (예: 불닭볶음면, 스팸, 참치캔, 슬라이스치즈).
등록한 재료에만 쿠팡 카드와 대가성 문구가 떠요. 카드에는 "AD" 표시가 붙어서 광고 영역으로 구분돼요.
`image`는 비워도 되고(재료 아이콘으로 대체), 쿠팡 상품 위젯 코드 안의 `productImage=` 값(URL 인코딩된 이미지 주소)을 넣으면 상품 썸네일이 떠요.
